...
const JournalView: React.FC = () => {
  ...
  const [viewState, setViewState] = useState<"list" | "edit" | "create">(
    "list",
  );
  ...
  const getMoodIcon = (mood?: string) => {
    switch (mood) {
      case "lucky":
        return "fa-star text-hb-gold";
      case "tilted":
        return "fa-fire text-red-500";
      default:
        return "fa-pen-nib text-hb-blue";
    }
  };
      ...
      <div className="p-5 min-h-full flex flex-col pb-24">
        <div className="mb-6 flex items-center justify-between">
          ...
          <button
            ...
            className="w-8 h-8 rounded-full bg-hb-surface text-hb-muted border border-hb-border flex items-center justify-center hover:text-white"
          >
        ...
        <div className="space-y-4 flex-1">
          <div>
            <label className="text-[10px] font-black uppercase text-hb-muted tracking-widest mb-1 block">
              Title
            </label>
            <input
          ...
          <div>
            <label className="text-[10px] font-black uppercase text-hb-muted tracking-widest mb-2 block">
              Current Mood
            </label>
            <div className="flex gap-2">
              {[
                ...
                <button
                  key={m.id}
                  onClick={() =>
                    setCurrentEntry({ ...currentEntry, mood: m.id as 'lucky' | 'neutral' | 'tilted' })
                  }
    ...
    <div className="p-5 pb-24 relative min-h-full">
      <div className="bg-gradient-to-r from-hb-surface to-hb-blueblack border border-hb-border p-8 rounded-[2rem] text-white mb-8 shadow-xl relative overflow-hidden">
        <div className="relative z-10">
          <h2 className="text-2xl font-black mb-1 italic tracking-tight uppercase text-hb-gold">
            Secure Journal
          </h2>
          <p className="text-[11px] font-bold uppercase tracking-widest opacity-70">
            Encrypted Player Notes
          </p>
        </div>
      ...
      {entries.length === 0 ? (
        <div className="text-center py-12 opacity-50">
          ...
          <p className="text-xs text-hb-muted mt-1">
            Start documenting your winning streaks!
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {entries.map((entry) => (
            <div
              key={entry.id}
              className="bg-hb-surface p-5 rounded-[1.5rem] border border-hb-border shadow-sm flex flex-col gap-3 group active:scale-[0.99] transition-transform"
            >
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="font-black text-white text-[14px] leading-tight mb-1">
                    {entry.title}
                  </h3>
                  <span className="text-[10px] text-hb-muted font-bold uppercase tracking-wide">
                    {entry.date}
                  </span>
                </div>
