...
const PaymentProofView: React.FC<PaymentProofViewProps> = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "model",
      text: "Welcome to ETHIOLOTTORY BINGO Support! How can I help you today? You can ask about deposits, withdrawals, or how to upload your payment proof.",
    },
  ]);
  ...
  const handleUpload = async () => {
    if (!file || !refId)
      return alert("Please select a file and enter a Reference ID.");
    ...
    try {
      ...
      setTimeout(() => {
        ...
        alert("Proof logged in our system! Our team will verify it shortly.");
        setFile(null);
        setRefId("");
        setNotes("");
      }, 1000);
      ...
      setIsUploading(false);
      alert("Failed to log proof. Please try again.");
    }
  };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const systemInstruction = `You are a helpful support agent for ETHIOLOTTORY BINGO, a betting app.
      Key Info:
      ...
      - If users ask about game rules: Mention it's a Bingo game with a 400-card deck and high entropy.
      ...
      setMessages((prev) => [
        ...
        {
          role: "model",
          text: "Sorry, I am offline right now. Please check your connection.",
        },
      ]);
    } finally {
      setIsTyping(false);
      ...
      {activeTab === "upload" && (
        <div className="flex-1 overflow-y-auto animate-in fade-in slide-in-from-bottom-2">
          <div className="bg-gradient-to-br from-hb-surface to-[#121212] p-8 rounded-[2rem] border border-hb-border text-white shadow-xl mb-6 relative overflow-hidden">
            <div className="relative z-10">
              <div className="flex items-center gap-3 mb-2">
                <img
                  src="https://is1-ssl.mzstatic.com/image/thumb/Purple211/v4/4a/6c/2e/4a6c2e37-122e-130f-2169-2810c9d94944/AppIcon-0-0-1x_U007emarketing-0-5-0-85-220.png/512x512bb.jpg"
              ...
              <p className="text-[11px] text-hb-muted font-bold leading-relaxed">
                Upload screenshots (Telebirr/CBE) or documents to verify your
                deposits manually.
              </p>
            </div>
          ...
          <div className="space-y-4">
            <div>
              <label className="text-[10px] font-black uppercase text-hb-muted tracking-widest mb-1.5 block ml-1">
                Transaction Ref / ID
              </label>
              <input
                type="text"
      ...
      {activeTab === "chat" && (
        <div className="flex-1 flex flex-col bg-slate-50 border border-hb-border rounded-[2rem] overflow-hidden shadow-inner animate-in fade-in slide-in-from-right-2">
          <div className="bg-white p-4 border-b border-hb-border flex items-center gap-3 shadow-sm">
            ...
            <div>
              <div className="text-sm font-black text-hb-navy">
                Support Team
              </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.map((msg, idx) => (
              ...
              >
                <div
                  className={`max-w-[80%] p-3.5 rounded-2xl text-xs font-bold leading-relaxed shadow-sm 
